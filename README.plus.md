...
- ✅ Works fully offline — no sign-in required to use the timer.
- ✅ All sessions are saved locally and synced when you sign in. Nothing is lost.
- ✅ See your pattern — a focus chart visualizes daily totals; click any block for the breakdown of completed pomodoros.
- ✅ Always-on motivation — a motivational quote is always one click away from the main nav, whether or not you're signed in.
- ✅ Account-only avatar — Open Peeps avatar builder is available only after signing in. Anonymous users see a placeholder.

## Offline behavior

- `/app` is fully functional without sign-in: timer, categories, and local history all work offline.
- `/` loads from cache when offline; the global feed is replaced by a notice.
- `/u/[username]` is online-only for public profiles.
- Anonymous sessions are written under a local key and merged into your account on sign-in via `claimOrphaned`.
